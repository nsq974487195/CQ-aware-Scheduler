...
static void bfq_dispatch_insert(struct request_queue *q, struct request *rq)
{
	...
	elv_dispatch_sort(q, rq);
 	...
 	rq->prev_read_size = bfqd->rq_in_device_size[0];
        rq->prev_write_size = bfqd->rq_in_device_size[1];
        bfqd->rq_in_device_size[rq_data_dir(rq)] += blk_rq_sectors(rq);
...
static void bfq_completed_request(struct request_queue *q, struct request *rq)
{
	...
	bfqd->rq_in_device_size[rq_data_dir(rq)] -= blk_rq_sectors_orig(rq);

	cur_time = sched_clock();

        d2c = cur_time - rq_io_start_time_ns(rq);
	trace_cq_reqinfo((bfq_bfqq_sync(bfqq) ? rq_data_dir(rq) : 2), blk_rq_bytes_orig(rq), rq_io_start_time_ns(rq) - rq_start_time_ns(rq), d2c, rq->prev_read_size, rq->prev_write_size);
