		...
		read_descriptor_t *desc, read_actor_t actor)
{
	...
	struct request_queue *dest_q = NULL;
    int do_more_ra = 0;
	...
	/* get storage queue to check storage device status */
    if(inode != NULL)
        if(inode->i_sb != NULL)
            if(inode->i_sb->s_bdev != NULL)
                dest_q = bdev_get_queue(inode->i_sb->s_bdev);

    if(dest_q != NULL)
    {
        if(dest_q->do_aggr_ra == 1)
        {
            if((dest_q->nr_rqs[0] >= 4) && (dest_q->sync_queues < 1))
            {
                do_more_ra = 1;
            }
        }
    }
    ra->do_more_ra = do_more_ra;
